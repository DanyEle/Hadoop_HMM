...
public class HMM<O> implements SequenceLabeler<O> {
    ...
    /**
     * Modified by Daniele Gadler - remove the null values in the observation
    ...
    {        
        ...
        for (int i = 0; i < observations.length; i++) 
        {
        	//observation is a single sequence in my case
        	O[] observation = observations[i];
        	O[] obsNoNull = removeNullValues(observation);
        	if(obsNoNull.length >= 2)
        	{
        		int[] obsTranslated =  translate(obsNoNull);
        		validObservations.add(obsTranslated);
        	}
        }
        
        int[][] obs = new int[validObservations.size()][];

        //now take the arraylist content and put it into the obs.
        for(int i = 0; i < validObservations.size(); i++)
    	{
        	obs[i] = validObservations.get(i);
    	}
    ...
    {
    	List<O> obsNonNull = new ArrayList<O>();
    	
    	for(int i = 0; i < observation.length;i++)
    	{
    		if(observation[i] != null)
    		{
    			obsNonNull.add(observation[i]);
    		}
    	}

    	O[] obsArray = (O[]) obsNonNull.toArray();
